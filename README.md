...
topBar.Position               = UDim2.new(0.5, -150, 0, 14)
...
addCorner(pinBtn, 12)
addStroke(pinBtn, Color3.fromRGB(90, 90, 90), 1.5)
...
local function updatePinVisuals()
	if isPinned then
		pinBtn.BackgroundColor3 = Color3.fromRGB(30, 60, 120)
		pinBtn.TextColor3       = Color3.fromRGB(100, 180, 255)
		for _, child in ipairs(pinBtn:GetChildren()) do
			if child:IsA("UIStroke") then
				child.Color = Color3.fromRGB(60, 120, 200)
			end
		end
	else
		pinBtn.BackgroundColor3 = Color3.fromRGB(50, 50, 50)
		pinBtn.TextColor3       = Color3.fromRGB(180, 180, 180)
		for _, child in ipairs(pinBtn:GetChildren()) do
			if child:IsA("UIStroke") then
...
do
	...
	topBar.InputBegan:Connect(function(input)
		...
		if onRect(pos, toggleBtn)
			or (lockBtn.Visible and onRect(pos, lockBtn))
			or (pinBtn.Visible and onRect(pos, pinBtn)) then
			return
		end
		state.touch       = input
		state.startPos    = pos
...
local function isOnUI(pos)
	local elements = {
		upBtn, downBtn, lockBtn, pinBtn, speedDown, speedUp,
